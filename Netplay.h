...
void NetWorkLoop()
{
	if (!isClient)
	{
		...
		//Discord
		discord_message("There's a JFK mario world server version " + GAME_VERSION + " being hosted, the ip is **" + sf::IpAddress::getPublicAddress(sf::seconds(5.f)).toString() + ":" + to_string(PORT) + "**");
