...
public:
	...
	//TO-DO: Globalize the pad variable. This code was copypasted from the player code.
	bool pad[8];
	bool pad_s[8];
	bool pad_p[8];
	void GetInput()
	{
		...
		if (!RAM[0x3F11] && !Chatting)
		{
			...
			pad[button_start] = state[input_settings[9]] || BUTTONS_GAMEPAD[8];
		}
		for (uint_fast8_t i = 0; i < 8; i++)
		{
			if (pad_s[i] != pad[i])
	...
	void Process()
	{
		...
		if (pad_p[button_start])
		{
			if (FreeCam)
			{
				if (!ResetCam)
				{
					...
					double gotoY = PositionY - (104.0 - 32.0);
		...
		*/
		...
		else
		{
			...
			*/
			if (!ResetCam) {
				CamX += (pad[button_right] - pad[button_left]) * 2;
				CamY += (pad[button_down] - pad[button_up]) * 2;
			}
			else {
				CamX += CamSX;
				CamY += CamSY;
