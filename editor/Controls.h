...
void save()
{
	if (current_file != "")
	{
		FILE* save_file = fopen(current_file.c_str(), "w");
		fprintf(save_file, ExportLevelToString().c_str());
		fclose(save_file);
...
void newFile()
{
	...
	std::ofstream out(path + "new_level/level_lua.lua");
	out << "function Init()\n	asm_write(1, 0x1412, 1)\n	asm_write(1, 0x1411, 1)\n\n	asm_write(8, 0x3F06, 1)\n	asm_write(8, 0x3F07, 2)\nend\n\nfunction Main()\nend";
	out.close();

	current_file = path + "new_level/level_data.txt";
...
void handle_controls()
{
	...
	if (current_file != "")
	{
		...
		if (picked_sprite >= 0)
		{
			...
			int p_y = snap ? ((mouse_y >> 4) << 4) + 8 : mouse_y;
